          ...
          <div className="grid gap-3.5">
            <div className="grid grid-cols-3 gap-6 rounded-xl border border-[#E5E8EE] bg-white p-[20px_24px]">
              <div>
                ...
                <Link href="/checkout/address" className="mt-2.5 inline-block font-semibold text-xs text-[#2563EB] no-underline">
                  Change
                </Link>
              </div>
              <div>
                ...
                <div className="text-[13px] leading-relaxed font-semibold">
                  ...
                  <br />
                  <span className="font-normal text-[#48505E]">2 sub-orders · arrives Wed 30 Jul</span>
                </div>
                <Link href="/checkout/shipping" className="mt-2.5 inline-block font-semibold text-xs text-[#2563EB] no-underline">
                  Change
                </Link>
              </div>
              <div>
                ...
                <div className="text-[13px] leading-relaxed font-semibold">
                  PayHere · Visa •••• 4291
                  <br />
                  <span className="font-normal text-[#48505E]">+ Rs 1,820 wallet</span>
                </div>
                <Link href="/checkout/payment" className="mt-2.5 inline-block font-semibold text-xs text-[#2563EB] no-underline">
                  Change
                </Link>
              </div>
            </div>
            ...
            {CART_GROUPS.map((g) => (
              <div key={g.seller} className="overflow-hidden rounded-xl border border-[#E5E8EE] bg-white">
                <div className="flex items-center gap-2.5 border-b border-[#EEF0F4] bg-[#FAFBFC] p-[14px_22px]">
                  ...
                  <span className="ml-auto font-mono text-[11.5px] text-[#98A1AF]">{g.note}</span>
                </div>
                {g.items.map((i) => (
                  <div
                    key={i.sku}
                  ...
                  >
                    ...
                    <div className="text-[12.5px] font-medium text-[#8A93A3]">× {i.qty}</div>
                    <div className="text-right font-bold text-sm">Rs {i.total}</div>
                  </div>
                ))}
              </div>
            ))}
            ...
            <div className="flex items-center gap-3 rounded-[11px] bg-[#E7F5EF] p-[15px_20px]">
              ...
              <div className="text-[12.5px] leading-relaxed text-[#0E6B4E]">
                Stock and prices were re-validated 8 seconds ago. Nothing changed since you added
                these items (FR-CART-005).
              </div>
            </div>
          </div>

          <div className="sticky top-6 rounded-xl border border-[#E5E8EE] bg-white p-[20px_22px]">
            <div className="mb-4 font-bold text-[14.5px]">Payment breakdown</div>
            {CART_SUMMARY.map((r) => (
            ...
            <div className="mb-2.5 flex justify-between">
              ...
              <span className="text-[13px] font-semibold text-[#12805C]">− Rs 1,820</span>
            </div>
            <div className="mt-1 flex items-baseline justify-between border-t border-[#EEF0F4] pt-3.5">
              ...
              <span className="font-extrabold text-xl">Rs 19,500</span>
            </div>
            <Link
              href="/order/confirmation"
              className="mt-4 block w-full cursor-pointer rounded-[9px] border-none bg-[#12805C] py-4 text-center font-bold text-sm text-white no-underline hover:bg-[#0E6B4E]"
            >
