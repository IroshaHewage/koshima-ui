...
export default function CategoryPage() {
  const gridProducts = [
    ...PRODUCTS,
    ...PRODUCTS.slice(0, 8 - (PRODUCTS.length % 8 || 8)),
  ].slice(0, 8)
      ...
      <div className="mx-auto max-w-[1360px] px-7 pt-[18px]">
        ...
        <div className="mb-[22px] flex items-center justify-between gap-[30px] rounded-2xl bg-[#FDF3F0] p-[30px_34px]">
          <div>
            ...
            <div className="max-w-[520px] text-sm leading-relaxed text-[#8A6258]">
              3,240 products from 412 verified sellers. Serums, cleansers,
              sunscreen and Ayurvedic preparations formulated for tropical
              humidity.
            </div>
          </div>
          <div className="relative aspect-video w-[220px] flex-none overflow-hidden rounded-[10px]">
            <Image
              src={storefrontImg("cat-1")}
              alt="Skincare"
              fill
        ...
        <div className="mb-[22px] flex flex-wrap gap-2">
          {SUB_CATS.map((c) => (
            <span
              key={c}
              className="cursor-pointer rounded-full border border-[#E5E8EE] bg-white px-4 py-2.5 text-[12.5px] font-semibold hover:border-[#101725]"
            >
              {c}
            </span>
          ))}
        ...
        <div className="grid grid-cols-[250px_1fr] items-start gap-6">
          <div className="overflow-hidden rounded-xl border border-[#E5E8EE] bg-white">
            <div className="flex items-center justify-between border-b border-[#EEF0F4] px-[18px] py-[15px]">
              ...
              <span className="cursor-pointer text-[11.5px] font-semibold text-[#2563EB]">
                Clear all
              </span>
            </div>
            <div className="border-b border-[#EEF0F4] px-[18px] py-4">
              <div className="mb-3.5 text-xs font-semibold">Price (LKR)</div>
              <div className="relative mx-1 mb-3.5 h-[3px] rounded-full bg-[#EEF0F4]">
                <div className="absolute top-0 right-[38%] bottom-0 left-[12%] rounded-full bg-[#2563EB]" />
              ...
              <div className="flex gap-2">
                <div className="flex-1 rounded-[7px] border border-[#E5E8EE] px-2.5 py-2 font-mono text-xs font-medium text-[#48505E]">
                  500
                </div>
                <div className="flex-1 rounded-[7px] border border-[#E5E8EE] px-2.5 py-2 font-mono text-xs font-medium text-[#48505E]">
                  8,000
            ...
            {FACETS.map((f) => (
              <div
                key={f.name}
                className="border-b border-[#EEF0F4] px-[18px] py-4"
              >
                <div className="mb-3.5 text-xs font-semibold">{f.name}</div>
                {f.options.map((o) => (
                  <div
                    key={o.label}
                    className="mb-2.5 flex cursor-pointer items-center gap-2.5"
                  >
                    <div
                    ...
                    <span className="flex-1 text-[12.5px] text-[#48505E]">
                      {o.label}
                    </span>
                    <span className="font-mono text-[11px] text-[#B3BAC6]">
                      {o.n}
                    </span>
                  </div>
                ))}
            ...
            <div className="px-[18px] py-[15px] font-mono text-[11.5px] leading-relaxed text-[#98A1AF]">
              Facets are generated from category attributes marked filterable
              (FR-CAT-006).
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-[26px] flex items-center justify-center gap-[7px]">
              <span className="rounded-lg bg-[#2563EB] px-[15px] py-[11px] text-[12.5px] font-semibold text-white">
                1
              </span>
              <span className="cursor-pointer rounded-lg border border-[#E5E8EE] bg-white px-[15px] py-[11px] text-[12.5px] font-semibold">
                2
              </span>
              <span className="cursor-pointer rounded-lg border border-[#E5E8EE] bg-white px-[15px] py-[11px] text-[12.5px] font-semibold">
                3
              ...
              <span className="ml-3.5 font-mono text-[11.5px] text-[#98A1AF]">
                server-side pagination · NFR-PERF-003
              </span>
            </div>
          </div>
        </div>
